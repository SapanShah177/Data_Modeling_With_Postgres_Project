...
def process_log_file(cur, filepath):
    ...
    for index, row in df.iterrows():
       # print(row.artist)
        #print(row.length)
        #query = f"SELECT song_id, artists.artist_id FROM songs JOIN artists ON songs.artist_id = artists.artist_id WHERE songs.duration = {row.length};"

        # get songid and artistid from song and artist tables
        demo = cur.execute(song_select, (row.song,row.artist, str(row.length)))
        #print(demo)
